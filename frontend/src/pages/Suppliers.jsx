...
import { Building2, Plus, Edit, X, Save, MapPin, Phone, Mail, RefreshCw, AlertCircle, Clock, FileText } from 'lucide-react';
...
const Suppliers = () => {
  ...
  useEffect(() => {
    if (showModal && user && !editingSupplier) {
  ...
  }, [showModal, user, editingSupplier]);
  ...
  const handleOpenModal = (supplier = null) => {
    if (supplier) {
      ...
      setFormData({
        ...
        business: supplier.business || user?.business_id || user?.business || '',
        branch: supplier.branch || user?.branch_id || user?.branch || ''
      });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingSupplier) {
        const response = await apiPut(`/supply-chain/suppliers/${editingSupplier.id}/`, submitData);
        ...
        const response = await apiPost('/supply-chain/suppliers/', submitData);
        ...
        showSuccess(`Supplier "${formData.name}" added successfully!`);
      }
        ...
        {!loading && !error && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-5 border-l-4 border-gray-600">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-600 mb-1">Inactive Suppliers</p>
                  <p className="text-3xl font-bold text-gray-800">
              ...
              <tbody className="divide-y divide-gray-200">
                {suppliers.map((supplier) => (
                  <tr key={supplier.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold capitalize ${getStatusBadge(supplier.status)}`}>
                        {supplier.status}
                      </span>
                    </td>
